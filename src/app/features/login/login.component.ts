...
export class LoginComponent implements AfterViewInit, OnDestroy{
    ...
    private setInitialPositions() {
      if (this.svgElements.armL && this.svgElements.armR) {
        gsap.set(this.svgElements.armL, { x: -93, y: 220, rotation: 105, transformOrigin: "top left" });
        gsap.set(this.svgElements.armR, { x: -93, y: 220, rotation: -105, transformOrigin: "top right" });
      }
    ...
    private calculateFaceMove(event: any) {
      ...
      const targetX = email.scrollWidth <= this.emailScrollMax
        ? this.emailCoords.x + caretCoords.x
        : this.emailCoords.x + this.emailScrollMax;
      ...
      // Apply animations
      this.animateFaceMovement(eyeLX, eyeLY, eyeRX, eyeRY, noseX, noseY, mouthX, mouthY, mouthR, mouthAngle);

      document.body.removeChild(div);
    }

    private animateFaceMovement(eyeLX: number, eyeLY: number, eyeRX: number, eyeRY: number,
                               noseX: number, noseY: number, mouthX: number, mouthY: number,
                               mouthR: number, mouthAngle: number) {
      ...
      gsap.to(this.svgElements.chin, 1, { x: -chinX, y: -chinY, scaleY: chinS, ease: "expo.out" });
      gsap.to(this.svgElements.face, 1, {
        x: -faceX, y: -faceY, skewX: -faceSkew,
        transformOrigin: "center top", ease: "expo.out"
      });
      gsap.to(this.svgElements.eyebrow, 1, {
        x: -faceX, y: -faceY, skewX: -eyebrowSkew,
        transformOrigin: "center top", ease: "expo.out"
      });
      ...
      gsap.to(this.svgElements.hair, 1, {
        ...
        transformOrigin: "center bottom", ease: "expo.out"
      });
    }

    private animateMouthSmall() {
      if (this.svgElements.mouthBG && this.svgElements.mouthOutline && this.svgElements.mouthMaskPath) {
        gsap.to([this.svgElements.mouthBG, this.svgElements.mouthOutline, this.svgElements.mouthMaskPath], 1, {
      ...
      gsap.to([this.svgElements.eyeL, this.svgElements.eyeR], 1, { scaleX: 1, scaleY: 1, ease: "expo.out" });
      this.eyeScale = 1;
    }
    ...
    private animateMouthMedium() {
      ...
      gsap.to([this.svgElements.eyeL, this.svgElements.eyeR], 1, { scaleX: 0.85, scaleY: 0.85, ease: "expo.out" });
      this.eyeScale = 0.85;
    }
    ...
    private coverEyes() {
      ...
      gsap.to(this.svgElements.armR, 0.45, { x: -93, y: 10, rotation: 0, ease: "quad.out", delay: 0.1 });
      gsap.to(this.svgElements.bodyBG, 0.45, { morphSVG: this.svgElements.bodyBGchanged, ease: "quad.out" });
    ...
    private uncoverEyes() {
      ...
      gsap.to(this.svgElements.bodyBG, 0.45, { morphSVG: this.svgElements.bodyBG, ease: "quad.out" });
      this.eyesCovered = false;
    }

    private resetFace() {
      gsap.to([this.svgElements.eyeL, this.svgElements.eyeR], 1, { x: 0, y: 0, ease: "expo.out" });
      ...
      gsap.to([this.svgElements.face, this.svgElements.eyebrow], 1, { x: 0, y: 0, skewX: 0, ease: "expo.out" });
      gsap.to([this.svgElements.outerEarL, this.svgElements.outerEarR, this.svgElements.earHairL, this.svgElements.earHairR, this.svgElements.hair], 1, {
        x: 0, y: 0, scaleY: 1, ease: "expo.out"
      });
    }
